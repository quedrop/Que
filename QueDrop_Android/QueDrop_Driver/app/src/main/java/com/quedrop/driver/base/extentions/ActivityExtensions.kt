...
package com.quedrop.driver.base.extentions
...
fun Activity.startActivityWithSharedElementTransition(intent: Intent, sharedElement: View) {
    ...
    statusBar?.let { listOfSharedElements.add(androidx.core.util.Pair(it, SHARED_ELEMENT_STATUS_BAR)) }
    navBar?.let { listOfSharedElements.add(androidx.core.util.Pair(it, SHARED_ELEMENT_NAVIGATION_BAR)) }
    sharedElement.let { listOfSharedElements.add(androidx.core.util.Pair(it, ViewCompat.getTransitionName(sharedElement).toString())) }
