package com.quedrop.driver.utils.customeCalender.views
...
class MonthView : ViewGroup, DatesGridLayout.CallBack {
    ...
    private lateinit var mMonthTitleTextView: View
    ...
    interface Callback {
        ...
        fun onSwipeLeft(isClick: Boolean)
        fun onSwipeRight(isClick: Boolean)
    }
    ...
    private fun initMonthTitle() {
        mMonthTitleTextView =
            mLayoutInflater.inflate(R.layout.zmail_layout_month_title, null) as View
        val calendar = Calendar.getInstance()
        calendar.set(mYear, mMonth, 1)
        val btnMonthTitle = mMonthTitleTextView.findViewById<TextView>(R.id.btnMonthTitle)
        btnMonthTitle.text =
            EventsCalendarUtil.getMonthString(calendar, EventsCalendarUtil.DISPLAY_STRING)
        btnMonthTitle.setTextColor(EventsCalendarUtil.primaryTextColor)
        if (EventsCalendarUtil.monthTitleTypeface != null) btnMonthTitle.typeface =
            EventsCalendarUtil.monthTitleTypeface
        btnMonthTitle.setTextColor(EventsCalendarUtil.monthTitleColor)
        btnMonthTitle.textSize = EventsCalendarUtil.monthTitleFontSize
        //Log.e("MONTH TEXT SIZE", EventsCalendarUtil.monthTitleFontSize.toString())

        val swipeLeft = mMonthTitleTextView.findViewById<ImageView>(R.id.ivSwipeLeft)
        swipeLeft.setOnClickListener {
            mCallback.onSwipeLeft(true)
        }
        val swipeRight = mMonthTitleTextView.findViewById<ImageView>(R.id.ivSwipeRight)
        swipeRight.setOnClickListener {
            mCallback.onSwipeRight(true)
        }
    ...
    private fun setWeekDayHeaderString(header: TextView, calendarConstant: Int) {
        ...
        when (calendarConstant) {
            ...
            Calendar.SUNDAY -> header.text = "Su"
            Calendar.MONDAY -> header.text = "Mo"
            Calendar.TUESDAY -> header.text = "Tu"
            Calendar.WEDNESDAY -> header.text = "We"
            Calendar.THURSDAY -> header.text = "Th"
            Calendar.FRIDAY -> header.text = "Fr"
            Calendar.SATURDAY -> header.text = "Sa"
        }
    }
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        mMonthTitleTextView.measure(
            widthMeasureSpec,
            MeasureSpec.makeMeasureSpec(
                resources.getDimension(R.dimen.height_month_title).toInt(),
                MeasureSpec.EXACTLY
            )
        )
        mWeekDaysHeader.measure(
            widthMeasureSpec,
            MeasureSpec.makeMeasureSpec(
                mContext.resources.getDimension(R.dimen.height_week_day_header).toInt(),
                MeasureSpec.EXACTLY
            )
        )
    ...
    override fun onLayout(changed: Boolean, l: Int, t: Int, r: Int, b: Int) {
        mMonthTitleTextView.layout(
            0,
            0,
            mMonthTitleTextView.measuredWidth,
            mMonthTitleTextView.measuredHeight
        )
        mWeekDaysHeader.layout(
            0,
            mMonthTitleTextView.measuredHeight,
            mWeekDaysHeader.measuredWidth,
            mMonthTitleTextView.measuredHeight + mWeekDaysHeader.measuredHeight
        )
        mMonthGridContainer.layout(
            0,
            mMonthTitleTextView.measuredHeight + mWeekDaysHeader.measuredHeight,
    ...
    fun onFocus(pos: Int) {
        if (pos == EventsCalendarUtil.monthPos) gridLayout.selectDefaultDate(
            EventsCalendarUtil.selectedDate.get(
