...
import com.quedrop.customer.ui.supplier.payment.viewmodel.AddEditPaymentViewModel
...
class AddEditPaymentActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (isEditMode) {
            ...
            tvBankTitle.text = "Edit Bank Details"
            btnAdd.text = "Save"
            setData()
        }
    }
    ...
    private fun init() {
        ivBankBack.throttleClicks().subscribe {
            onBackPressed()
        ...
        radioGroup.setOnCheckedChangeListener(object : RadioGroup.OnCheckedChangeListener {
            override fun onCheckedChanged(p0: RadioGroup?, p1: Int) {
                when (p1) {
                    radioSaving.id -> {
                        bankAccountType = ConstantUtils.BANK_ACCOUNT_SAVING

                    }
                    radioCurrent.id -> {
                        bankAccountType = ConstantUtils.BANK_ACCOUNT_CURRENT
                    }
                }
            }
    ...
    private fun setData() {
        orderDetail?.let {
            ...
            etAccountNumber.setText(it.account_number)
            etIfscCode.setText(it.ifsc_code)
            etAdditionalInfo.setText(it.other_detail)
            when (it.account_type) {
                ConstantUtils.BANK_ACCOUNT_SAVING -> {
                    radioSaving.isChecked = true
                }
                ConstantUtils.BANK_ACCOUNT_CURRENT -> {
                    radioCurrent.isChecked = true
                }
            }
            selectedBankId = it.bank_id!!
        }
    }
    ...
    private fun callAddPaymentMethodApi() {
        ...
        val jsonObject = JsonObject()
        if (isEditMode) {
            jsonObject.addProperty("bank_detail_id", orderDetail?.bank_detail_id)
        }
