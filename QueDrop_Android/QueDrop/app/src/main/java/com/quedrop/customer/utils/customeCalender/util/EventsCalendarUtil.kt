package com.quedrop.customer.utils.customeCalender.util
...
object EventsCalendarUtil {
    ...
    fun updateSelectedDates(c: Calendar) {
        if (datesInSelectedRange.contains(getDateString(c, DD_MM_YYYY))) {
            datesInSelectedRange.remove(getDateString(c, DD_MM_YYYY))
        } else {
            //datesInSelectedRange[getDateString(c, DD_MM_YYYY)]
            datesInSelectedRange.put(getDateString(c, DD_MM_YYYY), c)
        }
        Log.e("SIZE", getDateString(c, DD_MM_YYYY))
    }
    ...
    fun updateMinMaxDateInRange(c: Calendar) {
        when {
            ...
            c.before(minDateInRange) || areDatesSame(
                c,
                minDateInRange
            ) || minDateInRange == null -> {
                minDateInRange = c
    ...
    fun areDatesSame(date1: Calendar?, date2: Calendar?): Boolean =
        date1 != null && date2 != null && date1.get(Calendar.YEAR) == date2.get(Calendar.YEAR) && date1.get(
            Calendar.MONTH
        ) == date2.get(Calendar.MONTH) && date1.get(Calendar.DAY_OF_MONTH) == date2.get(Calendar.DAY_OF_MONTH)

    fun areMonthsSame(date1: Calendar?, date2: Calendar?): Boolean =
        date1 != null && date2 != null && date1.get(Calendar.YEAR) == date2.get(Calendar.YEAR) && date1.get(
            Calendar.MONTH
    ...
    fun isPastDay(date: Calendar): Boolean {
        if (date.get(Calendar.YEAR) < today.get(Calendar.YEAR)) return true
            ...
            else if (date.get(Calendar.MONTH) == today.get(Calendar.MONTH)) if (date.get(Calendar.DATE) < today.get(
                    Calendar.DATE
                )
            ) return true
        }
    ...
    fun getWeekCount(startDay: Calendar, endDay: Calendar): Int {
        ...
        while (!finished) {
            ...
            if (endDay.get(Calendar.MONTH) == helper.month && endDay.get(Calendar.YEAR) == helper.year) finished =
                true
            helper.nextMonth()
        }
        return noOfWeeks
    ...
    fun getMonthForWeekPosition(startMonth: Calendar, position: Int): Calendar {
        ...
        while (!finished) {
            offsetForPreviousMonth =
                ceil(((helper.offset + helper.numberOfDaysInMonth).toFloat() / 7.0f).toDouble()).toInt()
    ...
    fun getWeekData(fromDate: String): Array<String?> {
        ...
        if (fromDate.isNotEmpty()) {
            ...
            calendar.time = date
            for (i in 0..6) {
                days[i] = formatter.format(calendar.time)
                calendar.add(Calendar.DAY_OF_MONTH, 1)
                Log.e("CalendatUtil ", "==> " + days[i])
            }
        } else {
            Log.e("CalendatUtil ", "==> " + "fromDate isEmpty()")
        }
        return days
    }
