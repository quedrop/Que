...
@objc protocol PastOrderVCDelegate:NSObjectProtocol{
...
class PastOrderVC: UIViewController {
    ...
    //VARIABLES
    var delegate:PastOrderVCDelegate?
    var tableHeight = CGFloat(600.0)
    var arrPastOrder : [OrderDetail] = []
    ...
    extension PastOrderVC : UITableViewDelegate, UITableViewDataSource{
        ...
        func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            ...
            case enumOrderCells.topStoreDetails.rawValue:
                ...
                if arrPastOrder[indexPath.section].deliveryOption == DELIVERY_OPTION.Standard.rawValue {
                ...
                cell.btnRating.setTitle(" ⭐️ \(arrPastOrder[indexPath.section].rating ?? 0.0) ", for: .normal)
            ...
            case enumOrderCells.amountDetails.rawValue:
                ...
                cell.lblOrderAmount.text  = "\(Currency)\(arrPastOrder[indexPath.section].orderTotalAmount ?? 0)"
                cell.outerView.clipsToBounds = true
                cell.outerView.layer.cornerRadius = 8
                ...
                cell.selectionStyle = .none
                cell.btnStatus.setTitle("  \(arrPastOrder[indexPath.section].orderStatus ?? "")  ", for: .normal)
                return cell
            default:
                break
            }
        ...
        func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
            if (self.delegate?.responds(to: #selector(self.delegate?.naviagteToDetail(pastOrder:))))! {
                self.delegate?.naviagteToDetail(pastOrder: arrPastOrder[indexPath.section])
            }
       }
