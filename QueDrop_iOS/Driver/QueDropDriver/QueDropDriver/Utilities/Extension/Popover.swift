...
//  WeMeetApp
//
//  Created by C100-76 on 03/12/19.
//  Copyright © 2019 Matthias Weissenbacher. All rights reserved.
//
...
open class Popover: UIView {
    ...
    open func show(_ contentView: UIView, point: CGPoint, inView: UIView) {
		...
		self.arrowSize = CGSize(width: 0, height: 0)
        self.arrowShowPoint = CGPoint(x: point.x / 2.5 , y: point.y)
        self.show()
    }
    ...
    @objc open func dismiss() {
        if self.superview != nil {
			self.popOverDelegate?.PopOverdismiss()
			self.blackOverlay.alpha = 0
    ...
    override open func draw(_ rect: CGRect) {
        ...
        case .up:
            ...
            arrow.addLine(
                to: CGPoint(
                    x: arrowPoint.x - self.arrowSize.width * 0.4,
...
private extension Popover {
    ...
    func show() {
        ...
        //self.contentView.frame = CGRect(x: self.contentView.frame.minX - 10, y: self.contentView.frame.minY - 10, width: self.contentView.frame.width + 20, height: self.contentView.frame.height + 20)
       ...
       /*
        ...
        let contactRect = CGRect(x: -shadowSize, y: self.contentView.frame.height - (shadowSize * 0.4), width:self.contentView.frame.width + shadowSize * 2, height: shadowSize)
        self.contentView.layer.shadowPath = UIBezierPath(ovalIn: contactRect).cgPath
        self.contentView.layer.shadowRadius = 5
        self.contentView.layer.shadowOpacity = 0.4*/
       // self.contentView.bounds = CGRect(origin: CGPoint(x: 0, y: 0), size: CGSize(width: 50.0, height: 50.0))
