...
//  Pods
//
//  Created by Krishna on 29/12/17.
//

import Foundation
import UIKit

public class KPToast: NSObject {
    ...
    fileprivate static let shared: KPToast = {
        let instance = KPToast()
        return instance
    }()
    ...
    fileprivate func hideToastView() {
        if toastView != nil && toastView?.alpha == 1 {
            ...
            UIView.animate(withDuration: 0.15, delay: 2, options: [.allowUserInteraction, .curveEaseIn], animations: {
