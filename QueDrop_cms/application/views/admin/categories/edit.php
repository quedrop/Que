...
<!-- Content area -->
<div class="content">
	<form action="<?php echo base_url('admin/categories/edit/').$category['store_category_id']; ?>" id="categoryform" method="POST" enctype="multipart/form-data">
		<div class="row">		
						...
						<div class="row">
							<div class="col-md-12">
								<div class="form-group">
									...
									<input type="text" class="form-control" placeholder="<?php _el('category_name'); ?>" id="name" name="name" value="<?php echo $category['store_category_title']; ?>">
								</div>
								<div class="form-group">
									<small class="req text-danger"></small>
									<label><?php _el('cat_image'); ?>:</label>
									<input type="file" class="form-control" id="category_image" name="category_image" accept=".png, .jpg, .jpeg">
									<input type="hidden" name="old_image" id="old_image" value="<?php echo $category['store_category_image'];?>">
								</div>
								<div class="form-group">
									<small class="req text-danger">*</small>
									<label><?php _el('store_name'); ?>:</label>
									<select name="store_name" id="store_name" class="select">
										<option value="">Select Store</option>
										<?php
										if(!empty($store)){
											foreach($store as $st) {
											?>
											<option value="<?php echo $st['store_id']?>" <?php if($st['store_id'] == $category['store_id']){echo "selected";}?>><?php echo $st['store_name'];?></option>
											<?php
											}
										}
										?>
								...
								<div class="form-group">
									<label><?php _el('status'); ?>:</label>	
										<input type="checkbox" class="switchery" name="is_active" id="<?php echo $category['store_category_id']; ?>" <?php if ($category['is_active'] == 1) {
										echo "checked";
										}  ?>>								
								</div>	
							</div>
